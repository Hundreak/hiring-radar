...
(cd frontend && npm run check:duplicates && npm run check:component-size && npm run check:runtime-ux && npm run check:pagination-ux && npm run check:data-recovery && npm run check:auth-session-ux && npm run check:notification-center && npm run check:performance)
