...
from hiring_radar.services.ai.contracts import RoleFocusSuggestion, RoleFocusSuggestionResponse
...
class StubTask:
    ...
    def run(self, *, profile, locale: str, suggestion_count: int):
        assert locale == "tr"
        assert suggestion_count == 4
        return RoleFocusSuggestionResponse(
            role_suggestions=[
                RoleFocusSuggestion(
                    role_name="Embedded Systems Engineer",
                    fit_reason="Mevcut deneyim ve beceri seti ile güçlü uyum gösteriyor.",
                    missing_signals=["Daha görünür proje örnekleri"],
                    priority=1,
                )
            ],
            warnings=[],
...
def test_user_ai_role_focus_endpoint_returns_structured_suggestions(monkeypatch) -> None:
    app = create_app()
    ...
    monkeypatch.setattr(user_ai_module, "RoleFocusSuggestionTask", StubTask)

    client = TestClient(app)
    response = client.post(
        "/api/user/ai/profile/role-focus",
        json={"locale": "tr", "suggestion_count": 4},
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["role_suggestions"][0]["role_name"] == "Embedded Systems Engineer"
    assert payload["role_suggestions"][0]["priority"] == 1
