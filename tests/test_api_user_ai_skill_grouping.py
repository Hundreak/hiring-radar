...
from hiring_radar.services.ai.contracts import SkillGroup, SkillGroupingSuggestionResponse
...
class StubTask:
    ...
    def run(self, *, profile, locale: str, group_limit: int, skill_limit_per_group: int):
        assert locale == "tr"
        assert group_limit == 5
        assert skill_limit_per_group == 8
        return SkillGroupingSuggestionResponse(
            groups=[
                SkillGroup(
                    group_name="Embedded & Firmware",
                    skills=["C", "STM32", "RTOS"],
                    notes="Core low-level systems cluster.",
                )
            ],
            duplicates=["C / C Language"],
            normalization_suggestions=["Use one canonical label for C."],
            warnings=[],
        )
...
def test_user_ai_skill_grouping_endpoint_returns_structured_suggestions(monkeypatch) -> None:
    app = create_app()
    ...
    monkeypatch.setattr(user_ai_module, "SkillGroupingSuggestionTask", StubTask)

    client = TestClient(app)
    response = client.post(
        "/api/user/ai/profile/skill-grouping",
        json={"locale": "tr", "group_limit": 5, "skill_limit_per_group": 8},
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["groups"][0]["group_name"] == "Embedded & Firmware"
    assert payload["duplicates"] == ["C / C Language"]
